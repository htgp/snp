...
open out_snp_tmp,">>out_snp_dub_alt.txt" or die  "open out-snp file failed...\n";
while(<snp>){
	print "***************************************************************\n";
	if(!/^CHROM/){
	chomp;
	...
	@samples=($s1_1,$s2_2,$s1_2,$s2_1,$s3_1,$s3_2);
	if($alt=~/,/){
		$alt=(split(',',$alt))[0];
		for $spl(@samples){
		$spl=&shorten($spl);
...
sub ref_alt_alt{
	if($alt=~/,/){
		...
		print "$alt1,$alt2 are split.\n";
		($s1_1,$s1_2,$alt,$ref)=adapt($s1_1,$s1_2);
		print "after adaption, ref:$ref,alt:$alt,$s1_1|$s1_2.\n";
		($s2_1,$s2_2,$alt,$ref)=adapt($s2_1,$s2_2);
		print "after adaption, ref:$ref,alt:$alt,$s2_1|$s2_2.\n";
		($s3_1,$s3_2,$alt,$ref)=adapt($s3_1,$s3_2);
		print "after adaption, ref:$ref,alt:$alt,$s3_1|$s3_2.\n";
		}
		}
sub adapt{
	($t1,$t2)=@_;
	...
	if(fisher_test(@judge_complex_ref_alt)>0.05){#ref:alt=1:1?
		return join(',',$t1a,sum($t1b,$t1c)),join(',',$t2a,sum($t2b,$t2c)),$alt1,$ref;
		}
	else{
		if(fisher_test(@judge_complex_alt_alt)>0.05){#not ref:alt=1:1;then alt:alt?
			return join(',',$t1b,$t1c),join(',',$t2b,$t2c),$alt2,$alt1;
		}
		if(fisher_test(@judge_complex_ref_ref)>0.05){
			if(fisher_test(@judge_complex_ref_ref) > fisher_test(@judge_complex_alt_alt)){
				if(sum($t1b,$t2b)>=sum($t1c,$t2c)){
					return join(',',$t1a,$t1b),join(',',$t2a,$t2b),$alt1,$ref;}
				else{return join(',',$t1a,$t1c),join(',',$t2a,$t2c),$alt2,$ref;}
			}
			}
	}
	}
